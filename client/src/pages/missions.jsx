    ...
    const addMission = async (event) => {
        ...
        if (titreMission !== '' && dateMission !== '' && posteMission !== '' && structureMission !== '') {
            ...
            try {
                ...
                if (!response.ok) {
                    window.alert('Erreur lors de l\'ajout de la mission : ' + (await response.json()).error);
                    ...
                    <form onSubmit={addMission}>
                        ...
                        <div className="adminMissionButtonWrapper">
                            ...
                            <div className="adminMissionItem">Structure :
                                <select onChange={e => { setStructureMission(e.target.value); console.log(e.target); }} name="" id="" required>
                                    <option value="" disabled selected hidden>Sélectionnez une structure</option>
                                    {allStructures.map(structure => (
                                        <option key={structure.id} value={structure.id}>{structure.nom}</option>
                                    ))}
                                </select>
                            </div>
                            <Button submit={true} text="Ajouter" />
                        </div>
                    </form>
                </div>
            )}
